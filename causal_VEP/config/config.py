...
def setup_db_coords(database_name, project_name, overwrite=True):
    ...
    if not op.exists(database_name):
        raise NotADirectoryError('The specified direcory {0} '
                                 'does not exist, please check '
                                 'database position'.format(database_name))
    if (not op.exists(op.join(database_name, 'db_brainvisa')) and
        not op.exists(op.join(database_name, 'db_freesurfer'))):
        warnings.warn('The specified directory lacks of one or more needed '
    ...
    if (not op.exists(op.join(database_name, 'db_brainvisa', project_name)) and
        ...
        warnings.warn('Project not detected in one ore more databases, '
                      'please check')
    ...
    if op.exists(op.join(save_dir, 'db_coords.json')):
        if not overwrite: raise ValueError('Coordinate file exist, to change '
                                           'the values set \'overwrite=True\'')
        else:
    ...
    print('Database coordinates saved in {0}'.format(
        op.join(save_dir, 'db_coords.json')))
    return
...
def read_db_coords():
    ...
    if op.exists(json_fname):
        ...
        raise FileExistsError('Database coordinates file do not exist, please '
                              'set them using \'setup_db_coords()\' function, '
                              'specifying the folder that contains BrainVISA '
                              'and FreeSurfer databases, and the project name')


if __name__ == '__main__':
    setup_db_coords('/home/rbasanisi/data', 'meg_causal', overwrite=True)
